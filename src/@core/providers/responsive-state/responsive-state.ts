...
export class ResponsiveState {
  ...
  private _forceRefresh$: BehaviorSubject<null> = new BehaviorSubject<null>(
    null
  );
  ...
  constructor(
    ...
    this._screenWidth = this.isEnabledForPlatform
      ? this._window.screen.width
      : 0;
    this._screenHeight = this.isEnabledForPlatform
      ? this._window.screen.height
      : 0;
    this._userAgent = this.isEnabledForPlatform
      ? this._window.navigator.userAgent.toLowerCase()
      : null;
    ...
    if (this.isEnabledForPlatform) {
      const _resize$ = combineLatest(
        ...
        this._forceRefresh$
      ).pipe(debounceTime(this._responsiveConfig.config.debounceTime));

      const _pixelRatio$ = fromEvent(this._window, "onload").pipe(
        defaultIfEmpty(undefined),
        startWith(this.getDevicePixelRatio())
      );

      const _device$ = fromEvent(this._window, "onload").pipe(
        defaultIfEmpty(undefined),
        startWith(this.getUserAgent())
      );

      const _userAgent$ = fromEvent(this._window, "onload").pipe(
        defaultIfEmpty(undefined),
        startWith(this.userAgentData())
      );

      const _orientation$ = fromEvent(this._window, "orientationchange").pipe(
        defaultIfEmpty(undefined),
      ...
      this.orientation$ = _orientation$.pipe(
        map(this.orientationDevice.bind(this))
      );
  ...
  public getDevicePixelRatio(): any {
    ...
    if (this.isEnabledForPlatform) {
      if (
        typeof this._window.screen.systemXDPI !== "undefined" &&
  ...
  public sizeObserver(): any {
    return this._windows !== null && this.isEnabledForPlatform
      ? this.getWidth("window")
      : 0;
  }
  ...
  public sizeOperations(): any {
    ...
    if (
      ...
      if (_breackpoints.xl.min <= _width) {
        ...
        _breackpoints.lg.max >= _width &&
        _breackpoints.lg.min <= _width
      ) {
        _sizes = "lg";
      } else if (
        _breackpoints.md.max >= _width &&
        _breackpoints.md.min <= _width
      ) {
        _sizes = "md";
      } else if (
        _breackpoints.sm.max >= _width &&
        _breackpoints.sm.min <= _width
      ) {
        _sizes = "sm";
  ...
  public testIs4k(): boolean {
    return this._screenHeight !== 0 && this._screenWidth !== 0
      ? this._screenHeight < this._screenWidth
        ? this._screenWidth > 3839
        : this._screenHeight > 3839
      : false;
  }
  ...
  public getUserAgent(): any {
    return this.isEnabledForPlatform
      ? this._window.navigator.userAgent.toLowerCase()
      : null;
  }
  ...
  public userAgentData(): any {
    ...
    return {
      ...
      ie_version: {
        name: _ieVersionState
          ? this.ieVersionDetect()
  ...
  public standardDevices(): any {
    if (this.isEnabledForPlatform) {
      if (REG_MOBILES.IPHONE.REG.test(this._userAgent)) {
        ...
        this.isMobile() &&
        REG_MOBILES.ANDROID.REG.test(this._userAgent)
      ) {
  ...
  public ieVersionDetect(): any {
    if (this.isEnabledForPlatform) {
      ...
      if (REG_IE_VERSIONS.MS_MSIE.REG.test(_userAgent)) {
        _ieVersion = parseInt(
          _userAgent.substring(msie + 5, _userAgent.indexOf(".", msie)),
          10
        );
        if (_ieVersion === 7) {
      ...
      if (REG_IE_VERSIONS.MS_TRIDENT.REG.test(_userAgent)) {
        ...
        _ieVersion = parseInt(
          _userAgent.substring(_rv + 3, _userAgent.indexOf(".", _rv)),
          10
        );
        if (_ieVersion == 11) {
  ...
  public browserName(): any {
    ...
    if (this.isEnabledForPlatform) {
      if (
        REG_SORT_NAMES.WEBKIT.REG.test(this._userAgent) &&
        REG_SORT_NAMES.CHROME.REG.test(this._userAgent) &&
        !REG_BROWSERS.IE.REG.test(this._userAgent)
      ) {
        _browserName = REG_BROWSERS.CHROME.VALUE;
      } else if (
        REG_SORT_NAMES.MOZILLA.REG.test(this._userAgent) &&
        ...
        REG_SORT_NAMES.SAFARI.REG.test(this._userAgent) &&
        REG_SORT_NAMES.APPLE_WEBKIT.REG.test(this._userAgent) &&
        !REG_SORT_NAMES.CHROME.REG.test(this._userAgent)
      ) {
  ...
  public isMobile(): boolean {
    ...
    if (this._userAgent !== null) {
      ...
      _result =
        (REG_MOBILES.GENERIC_REG_1.REG.test(_userAgent) &&
          this.isTablet() === false) ||
        (REG_MOBILES.GENERIC_REG_2.REG.test(_userAgent.substr(0, 4)) &&
          this.isTablet() === false);
    }
    return _result;
  }
  ...
  public isTablet(): boolean {
    ...
    if (this._userAgent !== null) {
      _result =
        REG_TABLETS.IPAD.REG.test(this._userAgent) ||
  ...
  public isSMART(): boolean {
    ...
    if (this._userAgent !== null) {
      _result =
        REG_SMARTS_TV.GENERIC_TV.REG.test(this._userAgent) ||
  ...
  public isGameDevice(): boolean {
    ...
    if (this._userAgent !== null) {
      _result =
        REG_GAME_DEVICES.PS4.REG.test(this._userAgent) ||
        REG_GAME_DEVICES.PS3.REG.test(this._userAgent) ||
        REG_GAME_DEVICES.XBOX.REG.test(this._userAgent) ||
        REG_GAME_DEVICES.XBOX_ONE.REG.test(this._userAgent) ||
        REG_GAME_DEVICES.WII.REG.test(this._userAgent) ||
        REG_GAME_DEVICES.WII_U.REG.test(this._userAgent) ||
        REG_GAME_DEVICES.NINTENDO_3DS.REG.test(this._userAgent) ||
        REG_GAME_DEVICES.PS_VITA.REG.test(this._userAgent) ||
